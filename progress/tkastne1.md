...
**week 12**
I found A bug in my binary tree implementation with the grab function. The function was a complete mess, so I never found out what exactly was the problem with it, but it has since been replaced by much cleaner and logical code. To add to the completeness of the game I added power-ups to the game. For now there is a max hp and a weapon upgrade power-up. The max hp simply heals the player and gives them a short invincibility. The weapon upgrade will decrease the shot cooldown by 20% of all weapons currently being held by the player. Currently I'm thinking of adding a plus max hp power-up which would increase their max hp and a plus weapon count which could increase the max weapon count (to a limit (maybe)). And in hopes the make the game more fun, I made the max onscreen enemy count increase with the current level.